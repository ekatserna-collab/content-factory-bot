...
async function handleMessage(message) {
  ...
  if (text === "/start") {
    return tg("sendMessage", {
      ...
      text:
        `Привет, ${firstName}! 👋\n\n` +
        `Я бот-редактор канала <b>@aiplaceeee</b>. Сюда буду присылать черновики постов от агентов контент-завода. Ты решаешь — публиковать, отложить, доработать или отклонить.\n\n` +
        `Команды:\n` +
        `/claim — зарегистрироваться как владелец (нужно один раз)\n` +
  ...
  if (text === "/help") {
    return tg("sendMessage", {
      ...
      text:
        `Как пользоваться:\n\n` +
        `1. Агенты на серверах Anthropic 3 раза в день генерируют черновики и присылают их сюда.\n` +
        `2. На каждом черновике 5 кнопок:\n` +
        `   • ✅ В канал сейчас — публикация немедленно\n` +
        `   • ⏰ Завтра 09:00 — в очередь на завтрашнее утро\n` +
        `   • 🌆 Сегодня 19:00 — в очередь на сегодня вечером\n` +
        `   • ✏️ Доработать — следующим сообщением напиши правки, агент учтёт\n` +
        `   • ❌ Отклонить — отметится как отказанный\n\n` +
        `3. Vercel Cron каждый час проверяет очередь и публикует то, чему пора.`
    });
  }
